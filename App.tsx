...
const App: React.FC = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (now >= HOURLY_WINDOW_START && now <= HOURLY_WINDOW_END) {
        setUser(prev => {
          ...
          if (newPoints !== prev.points) return { ...prev, points: newPoints };
          return prev;
        });
      }
    }, 60000);
  ...
  const connectWallet = async () => {
    ...
    // 1. Logic for determining if we should skip selector
    if (!hasDetectedWallets && !fcAddr) {
      alert("No wallets detected. Please install a browser wallet like Coinbase Wallet or MetaMask.");
      return;
    }

    // 2. Decide: Immediate connect vs Modal
    // If we only have ONE option (either Farcaster OR a single browser wallet), proceed directly.
  ...
  const handleFarcasterAutoLogin = () => {
    const fcAddr = getFarcasterAddress(farcasterContextUser);
    if (!fcAddr) {
      alert("No linked wallet address found in your Farcaster profile. Please connect your wallet manually.");
      return;
    }
    setAddress(fcAddr);
  ...
  const handleConnectAndSign = async (provider: any) => {
    ...
    try {
      ...
      setIsConnecting(false);
      setIsSigning(false);
      alert("Connection or Signing failed. Please try again.");
    }
  };
  ...
  const handleConfirmVerification = async (challengeInput: string) => {
    ...
    try {
      ...
      if (success) {
        ...
        alert("Verification code not found. Please post the tweet first."); 
      }
    } catch (e) {
  ...
  const handleScan = async () => {
    ...
    if (!currentAddress) {
      alert("No active wallet connection found. Please connect your identity first.");
      return;
    }
    ...
    try {
      log("Analyzing footprint: " + currentAddress.slice(0, 8) + "...");
      setScanProgress(10);
      await sleep(400);

      log("Syncing assets from Base...");
      ...
      log("Crawling social history...");
      const scanResult = await twitterService.scanPosts(currentHandle).catch(() => ({ 
        accountAgeDays: 60, cappedPoints: 10, trustScore: 75, foundTweets: [] 
      }));
  ...
  const handleShare = (platform: 'farcaster' | 'twitter') => {
    ...
    const msg = `Base Impression Profile 🏎️💨\n\nRank: #${user.rank}\nPoints: ${user.points.toFixed(2)}\nTier: ${tierName}\n\nJoin the event:`;
    
    if (platform === 'twitter') {
      sdk.actions.openUrl(`https://twitter.com/intent/tweet?text=${encodeURIComponent(msg)}&url=${encodeURIComponent(shareUrl)}`);
    } else {
      sdk.actions.openUrl(`https://warpcast.com/~/compose?text=${encodeURIComponent(msg)}&embeds[]=${encodeURIComponent(shareUrl)}`);
    }
  };
  ...
  const claimStatus = useMemo(() => {
    ...
    if (!isRankEligible && !isAssetEligible) return { disabled: true, label: 'Ineligible', theme: 'bg-red-900/20 text-red-400', icon: AlertCircle, reason: 'Criteria not met.' };
    return { disabled: false, label: 'Mint Soulbound NFT', theme: 'bg-blue-600 shadow-lg text-white', icon: Award, reason: 'Eligible!' };
  }, [user]);

  const filteredLeaderboard = useMemo(() => {
    let list = [...MOCKED_LEADERBOARD];
    if (user && user.points > 0) {
      const handleStr = user.twitterHandle || `@${user.farcasterUsername}`;
      const idx = list.findIndex(l => l.handle === handleStr);
      if (idx !== -1) list[idx] = { ...list[idx], points: user.points, rank: user.rank };
      else list.push({ rank: user.rank, handle: handleStr, points: user.points, tier: getTierFromRank(user.rank), accountAgeDays: user.twitterAgeDays, baseAppAgeDays: user.baseAppAgeDays });
    }
  ...
  return (
    <div className="min-h-screen bg-black text-white font-['Space_Grotesk'] pb-24">
        ...
        <div className="fixed inset-0 z-[70] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="w-full max-w-sm glass-effect rounded-[3rem] p-8 border-blue-500/20 space-y-6">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3"><Cpu className="w-5 h-5 text-blue-500" /><h3 className="text-xl font-black uppercase italic tracking-tighter">Choose Source</h3></div>
      ...
      {showTwitterVerifyModal && (
        <div className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-md flex items-center justify-center p-6">
          <div className="w-full max-w-sm glass-effect p-8 rounded-[3rem] border-blue-500/30 space-y-6">
            <h2 className="text-xl font-black uppercase italic tracking-tighter">Identity Proof</h2>
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center font-mono font-black text-2xl text-blue-500">{twitterChallenge}</div>
            <div className="space-y-3">
              <button onClick={() => sdk.actions.openUrl(`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Verifying identity for Base Impression: ${twitterChallenge}`)}`)} className="w-full py-4 bg-white text-black rounded-2xl font-black uppercase text-[10px]">Post on X</button>
              <button onClick={() => handleConfirmVerification(twitterChallenge)} className="w-full py-4 bg-blue-600 rounded-2xl font-black uppercase text-[10px]">Check Status</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="sticky top-0 z-40 glass-effect px-4 py-4 flex justify-between items-center bg-black/60 backdrop-blur-md border-b border-white/5">
        <div className="flex items-center gap-3"><BrandLogo size="sm" /><div className="flex flex-col"><span className="text-[10px] font-black uppercase tracking-tighter leading-none">Base Impression</span><span className="text-[8px] font-bold text-blue-500 uppercase tracking-widest mt-0.5">Live Profile</span></div></div>
      ...
      <main className="max-w-md mx-auto px-4 mt-8">
        {!user ? (
          <div className="space-y-12 text-center animate-in fade-in duration-1000">
            ...
            <div className="space-y-3"><h1 className="text-5xl font-black uppercase italic tracking-tight leading-none">Onchain<br/>Impact.</h1><p className="text-[11px] text-gray-500 font-bold uppercase tracking-[0.3em]">Season 01 • Proof of Activity</p></div>
            ...
            <div className="glass-effect p-8 rounded-[3rem] space-y-6 shadow-2xl">
              <div className="space-y-4">
                <div className="space-y-2 text-left">
                  ...
                  {!isSignatureVerified ? (
                    <button onClick={connectWallet} className="w-full bg-blue-600/10 border border-blue-500/30 rounded-2xl py-4 px-5 flex items-center justify-between hover:bg-blue-600/20 transition-all">
                ...
                <div className="space-y-2 text-left">
                  ...
                  <div className="relative group flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input value={handle} onChange={e => setHandle(e.target.value)} disabled={isTwitterVerified} placeholder="@username" className="w-full bg-black/50 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-xs font-bold outline-none focus:border-blue-500 transition-all" />
                    </div>
                    {!isTwitterVerified ? (
                      <button onClick={handleStartTwitterVerification} className="px-6 bg-blue-600/20 border border-blue-500/30 rounded-2xl text-[9px] font-black uppercase hover:bg-blue-600/30 transition-all">Verify</button>
                    ) : (
              ...
              <button onClick={handleScan} disabled={isScanning} className="w-full py-5 bg-blue-600 rounded-[2rem] font-black uppercase italic text-sm shadow-xl shadow-blue-500/20 active:scale-95 disabled:opacity-20 flex items-center justify-center gap-2 transition-all">
                {isScanning ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Run Impression Audit'} <ArrowRight className="w-4 h-4" />
              </button>
            </div>
            <p className="text-[9px] text-gray-600 uppercase font-black tracking-widest italic">Secured by Base Onchain Graph</p>
          </div>
        ) : (
          <div className="space-y-8 animate-in slide-in-from-bottom-6 fade-in duration-700">
            ...
            {activeTab === 'dashboard' && (
              <div className="space-y-8">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="glass-effect p-6 rounded-[2.5rem] border-purple-500/20 text-center"><span className="text-[9px] font-black uppercase text-gray-500">Trust Index</span><div className="text-3xl font-black italic mt-1 text-blue-400">{user.trustScore}%</div></div>
                </div>

                <div className="glass-effect p-8 rounded-[3rem] space-y-4">
                   <div className="flex items-center justify-between">
                     <div className="flex items-center gap-3"><Fingerprint className="w-4 h-4 text-purple-400" /><h3 className="text-xs font-black uppercase italic">Ecosystem Status</h3></div>
                     {!user.farcasterId && <button onClick={handleSyncFarcaster} className="px-3 py-1.5 bg-purple-600/10 border border-purple-500/20 rounded-full text-[9px] font-black text-purple-400">Sync FC</button>}
                   </div>
                   <div className="grid grid-cols-2 gap-3">
                     <div className="p-3 bg-white/5 rounded-2xl border border-white/5 flex flex-col"><span className="text-[7px] font-black text-gray-500 uppercase">FID</span><span className="text-[10px] font-black">{user.farcasterId || 'Not Synced'}</span></div>
                     <div className="p-3 bg-white/5 rounded-2xl border border-white/5 flex flex-col"><span className="text-[7px] font-black text-gray-500 uppercase">Joined</span><span className="text-[10px] font-black">{user.farcasterCreatedAt || 'N/A'}</span></div>
                   </div>
                </div>
                ...
                <div className="glass-effect p-10 rounded-[4rem] text-center space-y-6">
                  ...
                  <div className="flex flex-col gap-3">
                    <button onClick={handleRefreshVisual} disabled={isGenerating} className="w-full py-4 bg-white text-black rounded-2xl font-black uppercase text-[10px] italic">Refresh Badge Visual</button>
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <button onClick={() => handleShare('twitter')} className="py-3 bg-blue-600/10 border border-blue-500/30 rounded-2xl text-[9px] font-black uppercase">Post to X</button>
                    </div>
                  </div>
                </div>
              </div>
                 ...
                 {filteredLeaderboard.slice(0, 50).map((l, i) => (
                   <div key={i} className={`p-6 glass-effect rounded-[2.5rem] flex justify-between items-center border border-white/5 hover:bg-white/5 transition-all`}>
                     <div className="flex items-center gap-4">
                       ...
                       <div className="flex flex-col"><span className="text-xs font-bold">{l.handle}</span><span className="text-[7px] font-black uppercase text-blue-400">{TIERS[l.tier].name} Level</span></div>
                     </div>
            ...
            {activeTab === 'claim' && (
              <div className="space-y-10 text-center py-6">
                 ...
                 <h2 className="text-3xl font-black uppercase italic">Impact Rewards</h2>
                 ...
                 <div className="space-y-4 px-4">
                   ...
                   <div className={`p-6 glass-effect rounded-[2rem] flex justify-between items-center ${user.lambolessBalance >= MIN_TOKEN_VALUE_USD ? 'border-green-500/30' : 'border-red-500/20'}`}>
                      <div className="text-left"><p className="text-[10px] font-black uppercase">$LAMBO Balance</p><p className="text-[8px] text-gray-500 font-bold uppercase">Min $2.50 USD</p></div>
                      <span className={`text-xs font-black ${user.lambolessBalance >= MIN_TOKEN_VALUE_USD ? 'text-green-400' : 'text-red-400'}`}>${user.lambolessBalance.toFixed(2)}</span>
                   </div>
                 </div>
                 <div className="space-y-4 px-4">
                   ...
                   <button disabled={claimStatus.disabled} className={`w-full py-6 rounded-[2.5rem] font-black uppercase italic text-sm transition-all ${claimStatus.theme}`}><claimStatus.icon className="w-5 h-5 inline mr-2" /> {claimStatus.label}</button>
                 </div>
              </div>
            )}
          </div>
