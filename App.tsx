...
const App: React.FC = () => {
  ...
  const standardWallets = [
    { name: "Coinbase Wallet", id: "coinbase", url: "https://www.coinbase.com/wallet", icon: "https://www.coinbase.com/favicon.ico" },
    { name: "OKX Wallet", id: "okx", url: "https://www.okx.com/web3", icon: "https://static.okx.com/cdn/assets/imgs/221/9E4E4E6E6B4E6B4E.png" },
    { name: "Zerion Wallet", id: "zerion", url: "https://zerion.io/", icon: "https://zerion.io/favicon.ico" },
    { name: "Rabby Wallet", id: "rabby", url: "https://rabby.io/", icon: "https://rabby.io/favicon.ico" }
  ];
            ...
            {loading ? (
              ...
              <div className="space-y-5">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {standardWallets.map(w => (
                          <button 
                            ...
                            className="p-3 bg-white/5 border border-white/5 rounded-xl flex flex-col items-center gap-2 hover:bg-white/10 transition-all active:scale-95"
                          >
                             <img src={w.icon} className="w-5 h-5 grayscale opacity-50" />
                             <span className="text-[8px] font-black uppercase tracking-tight text-gray-400">Install {w.name.split(' ')[0]}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                ...
                <div className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex items-start gap-3">
                  ...
                  <p className="text-[8px] text-gray-500 font-bold leading-normal uppercase">
                    Verification is gasless and read-only. We will never ask for your seed phrase or transfer permissions.
                  </p>
                </div>
              </div>
            )}
                    ...
                    <div className="space-y-3">
                        {[
                            ...
                            <div key={i} className="flex items-center justify-between p-5 glass-effect rounded-[1.5rem] border border-white/5 bg-white/[0.01] hover:bg-white/[0.03] transition-colors shadow-sm">
                                ...
                                <div className="text-right"><div className="text-xs font-black text-white">{stat.val}</div><div className="text-[9px] text-gray-600 font-black uppercase tracking-tighter">{stat.weight} Weight</div></div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {activeTab === 'leaderboard' && (
              <div className="space-y-6 animate-in fade-in duration-500">
                <div className="space-y-6">
                  ...
                  {/* Baseapp Account Age Filter */}
                  <div className="space-y-4">
                    <div className="flex items-center justify-between px-2"><span className="text-[9px] text-gray-500 font-black uppercase tracking-widest">Baseapp Seniority</span><div className="flex items-center gap-2 text-[9px] text-blue-400 font-black uppercase tracking-widest bg-blue-500/5 px-3 py-1.5 rounded-full border border-blue-500/10"><Smartphone className="w-3 h-3" />Age Filter</div></div>
                    <div className="flex gap-2 overflow-x-auto pb-1 px-1 scrollbar-hide">
                      {[{ key: 'ALL', label: 'All Time' }, { key: 'NEW', label: '< 1 Year' }, { key: 'ONE', label: '1 Year' }, { key: 'PLUS', label: '1+ Years' }].map((age) => (
                        <button key={age.key} onClick={() => setBaseAgeFilter(age.key as BaseAgeFilter)} className={`whitespace-nowrap px-5 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border active:scale-95 ${baseAgeFilter === age.key ? 'bg-blue-600 border-blue-400 text-white shadow-lg shadow-blue-600/20' : 'bg-white/5 border-white/5 text-gray-500 hover:text-gray-300 hover:border-white/10'}`}>{age.label}</button>
                      ))}
                    </div>
                  </div>
                </div>
